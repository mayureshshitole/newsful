...
const PoliticsNews = ({ poliNews }) => {
  return (
    <>
      <Head>
        <title>Politics News, Newsful- Raw.Real.Unbiased</title>
        <meta name="description" content="Bedhadak News website" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      < section className="mt-5 mx-auto px-1  sm:p-5 min-h-screen">
        <div>
          {poliNews.bedhadaknewsApi.map((newsItem, index) => (
            <div
              key={index}
...
export async function getStaticProps() {
  const { data: poliNews } = await client.query({
    query: gql`
      query {
